# Student Session Screen Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Improve the student session screen to show lesson content, session info, and a "raise hand" feature.

**Architecture:** Add new API endpoint for session content, update Prisma schema for handRaised field, refactor student session page with sidebar + content viewer + bottom panel layout.

**Tech Stack:** Next.js 16, React 19, Prisma, TypeScript, Tailwind CSS

---

## Task 1: Add handRaised field to Prisma schema

**Files:**
- Modify: `prisma/schema.prisma:133-146`

**Step 1: Add handRaised field**

In `prisma/schema.prisma`, add the new field to SessionAttendance:
...
git commit -m "feat(schema): add handRaised field to SessionAttendance"
```

---

## Task 2: Create API endpoint for session content

**Files:**
- Create: `src/app/api/student/session/content/route.ts`
...
Run dev server if not running, then test with browser or curl (requires valid student_token cookie).

**Step 3: Commit**

```bash
git add src/app/api/student/session/content/route.ts
git commit -m "feat(api): add student session content endpoint"
```

---

## Task 3: Create API endpoint for hand raise

**Files:**
- Create: `src/app/api/student/session/hand/route.ts`
...
```bash
git add src/app/api/student/session/hand/route.ts
git commit -m "feat(api): add hand raise endpoint"
```

---

## Task 4: Update student session page - Layout structure
...
Replace the existing interfaces and add new state at the top of the component:
...
```bash
git add src/app/(classroom)/class/session/page.tsx
git commit -m "feat(student-session): add new interfaces and state"
```

---

## Task 5: Update student session page - Data fetching

**Files:**
- Modify: `src/app/(classroom)/class/session/page.tsx`

**Step 1: Add content fetching logic**

After the existing useEffect for localStorage, add:
...
```bash
git add src/app/(classroom)/class/session/page.tsx
git commit -m "feat(student-session): add content fetching and helpers"
```

---

## Task 6: Update student session page - New UI layout

**Files:**
- Modify: `src/app/(classroom)/class/session/page.tsx`

**Step 1: Replace the ACTIVE render block**

Replace the entire `if (status === "ACTIVE" && sessionData)` block with the new layout:
...
```bash
git add src/app/(classroom)/class/session/page.tsx
git commit -m "feat(student-session): implement new UI layout with sidebar and content viewer"
```

---

## Task 7: Update teacher session page to show hand raised indicator

**Files:**
- Modify: `src/app/(dashboard)/dashboard/sessions/[sessionId]/page.tsx:25-36`

**Step 1: Update AttendanceRecord interface**

Add `handRaised` to the interface:
...
**Step 2: Add hand icon to student card**

In the student card rendering (around line 481), after the name section, add the hand indicator:
...
```bash
git add src/app/(dashboard)/dashboard/sessions/[sessionId]/page.tsx
git commit -m "feat(teacher-session): show hand raised indicator on student cards"
```

---

## Task 8: Update attendance API to include handRaised

**Files:**
- Modify: `src/app/api/sessions/[sessionId]/attendance/route.ts`

**Step 1: Check and update the query**

Read the current file and ensure the query includes `handRaised` in the select. The attendance endpoint should already return all fields, but verify and add if missing.

If the query uses `select`, add `handRaised: true`. If it returns all fields, no change needed.

**Step 2: Commit (if changes made)**

```bash
git add src/app/api/sessions/[sessionId]/attendance/route.ts
git commit -m "feat(api): include handRaised in attendance response"
```
...
1. Login as teacher, create a session
2. Login as student in another browser/incognito, join the session
3. Verify:
   - Student sees sidebar with resources
   - Student can click resources and view content
   - Timer shows elapsed time
   - Teacher name shows
   - "Levantar mano" button works
   - Teacher sees hand icon on student card when raised

**Step 3: Final commit**

```bash
git add -A
git commit -m "feat: complete student session screen with content viewer and hand raise"
```
